...
deploy_app() {
    ...
    aws cloudformation deploy \
        ...
        --capabilities CAPABILITY_IAM \
        --parameter-overrides "SESSMTPEndPoint=${SES_ENDPOINT}" "SESSMTPUserName=${SES_USER}" "SESSMTPPassowrd=${SES_PWD}" "EmailFrom=${EMAIL_FROM}" "ProjectName=${PROJECT_NAME}" "MeshOwner=${AWS_PRIMARY_ACCOUNT_ID}" "VPC=${SHARED_VPC}" "PrivateSubnet1=${SHARED_PRIVATE_SUBNET_1}" "PrivateSubnet2=${SHARED_PRIVATE_SUBNET_2}" "EnvoyImage=${ENVOY_IMAGE}" "DealerImage1=${DEALER_IMAGE_VER1}" "DealerImage2=${DEALER_IMAGE_VER2}"
}
