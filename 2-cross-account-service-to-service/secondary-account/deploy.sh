...
deploy_app() {
    ...
    aws cloudformation deploy \
        ...
        --capabilities CAPABILITY_IAM \
        --parameter-overrides "ProjectName=${PROJECT_NAME}" "MeshOwner=${AWS_PRIMARY_ACCOUNT_ID}" "VPC=${SHARED_VPC}" "PrivateSubnet1=${SHARED_PRIVATE_SUBNET_1}" "PrivateSubnet2=${SHARED_PRIVATE_SUBNET_2}" "EnvoyImage=${ENVOY_IMAGE}" "EmailImage=${EMAIL_IMAGE}"
}
